"""
fyk: filter the GT by the class we need.
['plane','ship','storage','harbor','bridge','vehicle']
"""
import os
import codecs
import numpy as np
...
class splitbase():
    ...
    def savepatches(self, resizeimg, objects, subimgname, left, up, right, down):
        ...
        out_lines = ''
        for obj in objects:
            if (self.filter_cls is not None) and (obj['name'] not in self.filter_cls):
                continue
...
if __name__ == '__main__':
    ...
    filter_cls = ['plane','ship','storage-tank','harbor','bridge','large-vehicle']
    split = splitbase(r'../DOTA/val',
                       r'../DOTA/val-split-800',
                       filter_cls=filter_cls, gap=200, subsize=800)
    split.splitdata(1)
    split = splitbase(r'../DOTA/train',
                       r'../DOTA/train-split-800',
                       filter_cls=filter_cls, gap=200, subsize=800)
    split.splitdata(1)
